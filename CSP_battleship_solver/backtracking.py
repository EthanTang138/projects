...
class UnassignedVars:
    ...
    def add_assigned(self, var):
        self.assigned.append(var)

    def rem_assigned(self, var):
...
def bt_search(algo, csp, variableHeuristic, allSolutions, trace):
    ...
    for v in uv.unassigned:
        if v.isAssigned():
            uv.unassigned.remove(v)
    if algo == 'BT':
...
def FC(unAssignedVars, csp, allSolutions, trace):
    ...
    for val in var.curDomain():
        unAssignedVars.rem_assigned(var)
        var.setValue(val)
        unAssignedVars.add_assigned(var)
        noDWO = True
...
def FCCheck(constraint, assignedvar, assignedval):
    ...
    for val in var.curDomain():
        ...
        if not constraint.check():
            ...
            pruned_vals.append(val)
        var.unAssign()

    if var.curDomainSize() == 0:
        return "DWO", pruned_vals
    return "OK", pruned_vals
...
def GacEnforce(constraints, assignedvar, assignedval, csp):
    while constraints:
