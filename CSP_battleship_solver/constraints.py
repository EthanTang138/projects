...
class NValuesConstraint(Constraint):
    ...
    def satisfied(self):
        ...
        return self._lb <= rv_count <= self._ub
...
class TwoCellConstraint(Constraint):
    """A table constraint with the values of domain1 and domain2"""

    def __init__(self, name, scope, satisfyingDomain1, satisfyingDomain2):
        Constraint.__init__(self, name, scope)
        self._name = "TwoCell_" + name
        satAssignments = []
        for val in satisfyingDomain1:
            for val2 in satisfyingDomain2:
                satAssignments.append([val, val2])

        self.satAssignments = satAssignments
